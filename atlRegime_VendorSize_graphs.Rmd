...
# undebug(LatticePercentLineWrapper)
VendorGraph<-LatticePercentLineWrapper(
    VAR.color.legend.label="Vendor Size"
    ,""#VAR.main.label=""#Overall DoD Vendor Size by Starting Regime"
...
for (c in unique(Policy$SubCustomer)){    
    ...
    VendorGraph<-LatticePercentLineWrapper(
        VAR.color.legend.label="Vendor Size"
        ,VAR.main.label=""#paste(c,"Vendor Size by Starting Regime")
        ,VAR.X.label="Month of Obligation (6 Month Lagging Average)"
...
for (s in unique(Policy$Vendor.Size.sum)){    
    png(
        paste( "Output\\Vendor_Size\\",
               CleanFileName(paste(
                   "DoD"
    ...
    Policy$Graph[Policy$Vendor.Size.sum!=s]<-FALSE
    Policy$Graph[Policy$Vendor.Size.sum==s]<-TRUE
    ...
    # VendorGraph<-VendorGraph+geom_rect(aes(xmin=StartDate, xmax=EndDate, ymin=-Inf, ymax=Inf),color="red",alpha=0,size=1)
    VendorGraph<-VendorGraph+coord_cartesian(ylim=c(0, 1))+scale_y_continuous(labels = percent_format())+
        theme(legend.position="bottom",axis.text.x = element_text(angle = 90, hjust = 1),legend.box='horizontal',
              legend.direction="vertical")+guides(color=guide_legend(ncol=2))
#     VendorGraph+theme(legend.direction="vertical")
    #     scale_x_date(labels=date_format("'%y"),breaks=date_breaks("year"))
    VendorGraph
    print(VendorGraph)
    #     VendorGraph+scale_fill_manual(values = alpha(c("red","blue"), .3))
...
OldGraph<-Policy$Graph
for (c in unique(Policy$SubCustomer)){ 
    VendorSubCustomer<-subset(Policy,SubCustomer==c)
    for (s in unique(Policy$Vendor.Size.sum)){    
...
write.csv(USDATLTable,paste("Output\\Vendor_Size\\",
                                  paste("DoD_components"
                                        ,"Obligation_2014"
                                        ,"Vendor_Size"
                                        ,"USDATL"
...
write.csv(PolicyTable,paste("Output\\Vendor_Size\\",
                                  paste("DoD_components"
                                        ,"Obligation_2014"
                                        ,"Vendor_Size"
                                        ,"ATLpolicy"
