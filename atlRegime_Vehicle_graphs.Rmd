...
VehicleGraph<-LatticePercentLineWrapper(
    VAR.color.legend.label="Contract Vehicle"
    ,""#VAR.main.label=""#Overall DoD Contract Vehicle by Starting Regime"
...
for (c in unique(Policy$SubCustomer)){    
    ...
    VehicleGraph<-LatticePercentLineWrapper(
        VAR.color.legend.label="Contract Vehicle"
        ,VAR.main.label=""#paste(c,"Contract Vehicle by Starting Regime")
        ,VAR.X.label="Month of Obligation (6 Month Lagging Average)"
...
for (c in unique(Policy$SubCustomer)){ 
    ...
    for (s in unique(Policy$Vehicle.sum)){    
        ...
        if(any(VehicleSubCustomer$Graph==TRUE)){
            ...
            # VehicleGraph<-VehicleGraph+geom_rect(aes(xmin=StartDate, xmax=EndDate, ymin=-Inf, ymax=Inf),color="red",alpha=0,size=1)
            # VehicleGraph<-VehicleGraph+coord_cartesian(ylim=c(0, 1))+scale_y_continuous(labels = percent_format())+
                VehicleGraph<-VehicleGraph+theme(legend.position="bottom",axis.text.x = element_text(angle = 90, hjust = 1),legend.box='horizontal',legend.direction="vertical")+guides(color=guide_legend(ncol=2))
            #     scale_x_date(labels=date_format("'%y"),breaks=date_breaks("year"))
            VehicleGraph
            print(VehicleGraph)
            #     VehicleGraph+scale_fill_manual(values = alpha(c("red","blue"), .3))
