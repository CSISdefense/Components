...
for (c in unique(CompetitionPolicy$SubCustomer)){    
    png(
    ...
    CompGraph<-LatticePercentLineWrapper(
        ...
        ,VAR.long.DF=subset(CompetitionPolicy,SubCustomer==c)
        ,VAR.ncol=NA
        ,VAR.x.variable="SignedMonth"
        ,VAR.y.variable="Obligation.2014"
        ,VAR.y.series=    "Competition.sum"
...
```{r PrepareHorseTail}
CompetitionPolicy$SinceFirstFiscalYear<-as.numeric(as.duration(
    ymd(CompetitionPolicy$SignedMonth)-CompetitionPolicy$StartFiscalYear)
    /dyears(1)
    )
...
OldGraph<-CompetitionPolicy$Graph

for (s in unique(CompetitionPolicy$Competition.sum)){    
    png(
        paste( "Output\\Competition\\",
               paste(
                   "DoD"
    ...
    CompetitionPolicy$Graph[CompetitionPolicy$Competition.sum!=s]<-FALSE
    CompetitionPolicy$Graph[CompetitionPolicy$Competition.sum==s]<-TRUE
    ...
    CompGraph<-LatticePercentLineWrapper(
        ...
        ,VAR.facet.primary="Competition.sum"
        ,VAR.facet.secondary=NA
        #     ,"StartDate"
        #     ,"EndDate"
        ,MovingAverage=3
...
CompetitionPolicy$Graph<-OldGraph
...
OldGraph<-CompetitionPolicy$Graph
for (c in unique(CompetitionPolicy$SubCustomer)){ 
    CompSubCustomer<-subset(CompetitionPolicy,SubCustomer==c)
    for (s in unique(CompetitionPolicy$Competition.sum)){    
        CompSubCustomer$Graph[CompSubCustomer$Competition.sum!=s]<-FALSE
        CompSubCustomer$Graph[CompSubCustomer$Competition.sum==s]<-TRUE
...
#Annual
CompPolicyTable<-ddply(CompetitionPolicy, c("Fiscal.Year"
                                         ,"Competition.sum"
                                         ,"SubCustomer.component"
                                         ,"ATLpolicy")
                         ,summarise
...
CompPolicyTable<-ddply(CompetitionPolicy, c("SignedMonth"
                                         ,"Competition.sum"
                                         ,"SubCustomer.component"
                                         ,"ATLpolicy")
                         ,summarise
